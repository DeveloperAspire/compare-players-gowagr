...
export default function PlayerPage() {
  ...
  const getPlayerStats = useMemo(
    () => (playerId: string, season: string, competition: string) => {
  ...
  useEffect(() => {
    setIsLoading(true)
    try {
      const stats = getPlayerStats(id as string, selectedSeason, selectedCompetition)
  ...
  }, [id, selectedSeason, selectedCompetition, getPlayerStats])
                  ...
                  {Object.entries(playerStats).map(([stat, value]) =>
                    typeof value !== "number" || stat === "id" ? null : (
                      <motion.p
                        ...
                        transition={{ duration: 0.3, delay: Object.keys(playerStats).indexOf(stat) * 0.1 }}
                      >
